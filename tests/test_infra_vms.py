...
@pytest.fixture
def pick_random_vm(home_page_logged_in):
...
class TestVirtualMachines:
    ...
    def test_vm_details(self, pick_random_vm):
        ...
        Assert.true(vm_details.details.get_section(
            'Properties').get_item('Name') != None)
    ...
    def test_set_vm_retirement_date(self, pick_random_vm):
        ...
        Assert.true(set_retirement_date_pg.date_field.get_attribute(
            'value') == retirement_date)
        vm_pg = set_retirement_date_pg.click_on_cancel()
        Assert.true(vm_pg.flash.message.startswith(
    ...
    def test_immediately_retire_vm(self, pick_random_vm):
        ...
        Assert.true(vm_pg.details.get_section("Properties").get_item(
            "Name").value == vm_name)        
